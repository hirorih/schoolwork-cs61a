...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if isinstance(insect, QueenAnt) and insect.the_queen:
                return
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        distance = 0
        while not curr_place.bees and curr_place.entrance is not hive:
...
# BEGIN Problem 7
class WallAnt(Ant):
    """A WallAnt that does nothing each turn but with its armor value 4"""
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower are waterdafe ThrowerAnt"""
...
class QueenAnt(ScubaThrower):
    ...
    def action(self, colony):
        ...
        """
        if not self.the_queen:
            super().reduce_armor(self.armor)

            return
        def double(place):

            if place.ant != None and type(place.ant) != QueenAnt:
                if place.ant not in self.doubled_list:
    ...
    def reduce_armor(self, amount):
        ...
        """
        self.armor -= amount
        if self.armor <= 0 and self.the_queen:
            bees_win()
