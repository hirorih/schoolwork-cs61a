...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    i, sum_scores, pigout = 0, 0, False
    while i < num_rolls:
        dice_outcome = dice()
        if dice_outcome == 1:
            pigout_score = dice_outcome
            pigout = True
        else:
            sum_scores += dice_outcome
        i += 1
    if pigout:
        return pigout_score
    else:
...
def free_bacon(score):
    ...
    if score >= 10:
        tens_digit = score // 10
    return max(ones_digit, tens_digit) + 1
    # END PROBLEM 2
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        ...
        if is_swap(score0, score1):
            swapped_score1 = score0
            score0 = score1
            score1 = swapped_score1
        player = other(player)
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def announce(score0, score1):
        ...
        if gain_score > previous_high:
            if gain_score == 1:
                print(gain_score, "point! That's the biggest gain yet for Player", who)
            else:
                print(gain_score, "points! That's the biggest gain yet for Player", who)
            return announce_highest(who, gain_score, pre_score)
        return announce_highest(who, previous_high, pre_score)
    return announce
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    avg = 0
    while rolls > 0:
        temp_avg = make_averaged(roll_dice)(rolls, dice)
        if temp_avg > avg:
            avg = temp_avg
            max_num_roll = rolls
        rolls -= 1
